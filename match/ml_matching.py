    ...
    def get_user_features(self, profile):
        ...
        try:
            features = np.array([
                ...
                float(profile.number_of_kids or 0),
                float({'N': 0, 'S': 1, 'R': 2}.get(profile.smoking, 0)),
                float({'N': 0, 'S': 1, 'R': 2}.get(profile.drinking, 0)),
                float({'NSR': 0, 'CAS': 1, 'LTR': 2, 'MAR': 3}.get(profile.relationship_goal, 0))
            ]).reshape(1, -1)
            return features
        except Exception as e:
            print(f"Error extracting features for profile {profile.id}: {str(e)}")
            return None