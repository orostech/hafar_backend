...
class MatchingService:
    def __init__(self, user):
        ...
        self.preference_weights = UserPreferenceWeight.objects.get_or_create(user=user)[0]
    ...
    def get_potential_matches(self, limit=100):
        ...
        # print(base_matches)
        base_profiles = [profile for profile, score in base_matches[:limit*2]]  # Get more for ML to filter
        
        # # Enhance matches using ML
        enhanced_matches = self.ml_service.enhance_matches(base_profiles, limit=limit)
    ...
    def _get_base_queryset(self):
        ...
        if self.user_profile.interested_in != 'E':
            base_queryset = base_queryset.filter(gender=self.user_profile.interested_in)

        # # Apply age range filtering
        # base_queryset = base_queryset.filter(
        #     date_of_birth__gte=date.today() - relativedelta(years=self.user_profile.maximum_age_preference),
        #     date_of_birth__lte=date.today() - relativedelta(years=self.user_profile.minimum_age_preference)
        # )
